...
class CrmAnalyticsService {
  static CrmAnalytics calculateAnalytics(List<Customer> customers) {
    ...
    final totalRevenue =
        customers.fold(0.0, (sum, c) => sum + c.computedTotalSpent);
    ...
    final regularCustomers =
        customers.where((c) => !c.isVip && c.visitCount > 0).length;
    ...
    final allCommunications =
        customers.expand((c) => c.communicationHistory).toList();
    ...
    final activeCustomers = customers.where((c) {
      if (c.computedLastVisit == null) return false;
      final daysSinceLastVisit = now.difference(c.computedLastVisit!).inDays;
      return daysSinceLastVisit <= 90; // Active within 90 days
    }).length;

    final dormantCustomers = customers.where((c) {
      if (c.computedLastVisit == null) return true;
      final daysSinceLastVisit = now.difference(c.computedLastVisit!).inDays;
    ...
    final topCustomers = List<Customer>.from(customers)
      ..sort((a, b) => b.computedTotalSpent.compareTo(a.computedTotalSpent));

    // Communication frequency analysis
    final customersWithCommunications =
        customers.where((c) => c.communicationHistory.isNotEmpty).length;
  ...
  static List<MonthlyGrowthData> _generateGrowthData(List<Customer> customers) {
    ...
    for (int i = 11; i >= 0; i--) {
      ...
      final revenue = customers
          .where(
              (c) => c.createdAt.isBefore(month.add(const Duration(days: 31))))
          .fold(0.0, (sum, c) => sum + c.computedTotalSpent);
